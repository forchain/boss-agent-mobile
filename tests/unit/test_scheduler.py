...
def test_is_cron_match():
    ...
    mon_9am = datetime(2026, 9, 7, 9, 0, 0, tzinfo=timezone.utc)
    mon_901am = datetime(2026, 9, 7, 9, 1, 0, tzinfo=timezone.utc)
    mon_10am = datetime(2026, 9, 7, 10, 0, 0, tzinfo=timezone.utc)
    # 2026-09-06 is Sunday
    sun_9am = datetime(2026, 9, 6, 9, 0, 0, tzinfo=timezone.utc)
    ...
    # 3. Every 15 minutes
    step_cron = "*/15 * * * *"
    assert is_cron_match(step_cron, datetime(2026, 9, 7, 14, 0, tzinfo=timezone.utc)) is True
    assert is_cron_match(step_cron, datetime(2026, 9, 7, 14, 15, tzinfo=timezone.utc)) is True
    assert is_cron_match(step_cron, datetime(2026, 9, 7, 14, 30, tzinfo=timezone.utc)) is True
    assert is_cron_match(step_cron, datetime(2026, 9, 7, 14, 45, tzinfo=timezone.utc)) is True
    assert is_cron_match(step_cron, datetime(2026, 9, 7, 14, 10, tzinfo=timezone.utc)) is False


def test_get_next_cron_run():
    # From 09:05 on Monday, next run of "0 10 * * *" should be 10:00 on same day
    base_dt = datetime(2026, 9, 7, 9, 5, 0, tzinfo=timezone.utc)
    next_run = get_next_cron_run("0 10 * * *", after=base_dt)
    assert next_run == datetime(2026, 9, 7, 10, 0, 0, tzinfo=timezone.utc)
...
async def test_scheduler_run_once():
    ...
    # Monday at 09:00
    monday_9am = datetime(2026, 9, 7, 9, 0, 0, tzinfo=timezone.utc)
    ...
    same_minute = datetime(2026, 9, 7, 9, 0, 30, tzinfo=timezone.utc)
    ...
    monday_2pm = datetime(2026, 9, 7, 14, 0, 0, tzinfo=timezone.utc)
