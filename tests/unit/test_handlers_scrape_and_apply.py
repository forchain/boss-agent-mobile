...
async def test_scrape_jobs_handler_clears_filters_when_enable_filter_false(broker, mock_driver):
    ...
    with patch("boss_agent.worker.handlers.scrape_jobs.FilterDialogPage") as mock_filter_cls:
...
async def test_auto_apply_handler_clears_filters_when_no_filter(broker, mock_driver):
    ...
    with (
        patch("boss_agent.worker.handlers.auto_apply.FilterDialogPage") as mock_filter_cls,
        patch("boss_agent.worker.handlers.auto_apply.run_job_application_graph") as mock_graph,
    ):
        mock_filter_inst = MagicMock()
        mock_filter_cls.return_value = mock_filter_inst
        mock_graph.return_value = MagicMock(
            app_rule_pass=False,
            app_rule_violation="skip",
