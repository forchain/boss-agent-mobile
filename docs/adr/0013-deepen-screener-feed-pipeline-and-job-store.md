...
- **Superseded (2026-09-25):** the transition wrappers are retired. `JobRecordStoreFacade` and the pipeline's `getattr(broker, "job_store", broker)` fallback were deleted, and every caller now names `broker.job_store` explicitly — the store is resolved once at composition time rather than by duck typing. Keeping the wrapper "for one transition cycle" had made it load-bearing for roughly 115 test call sites and nothing in production, which is the opposite of what the wrapper was for. Confinement of `BaseTaskBroker` to task lifecycle, leases, logs and subscriptions continues; its remaining Candidate Profile / Resume Revision and SavedSearch verbs are still to be extracted into their own repository seams.
EOF
